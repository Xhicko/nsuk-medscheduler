...
import { conditions1Schema } from '@/lib/medicalForms'

const fields = [
  { key: 'prev_tuberculosis', label: 'Tuberculosis' },
  { key: 'prev_hypertension', label: 'Hypertension (High blood pressure)' },
  { key: 'prev_epilepsy', label: 'Epilepsy' },
  { key: 'prev_mental_illness', label: 'Mental illness' },
]

const Conditions1 = forwardRef(function Conditions1({ formData = {}, onFormChange = () => {}, readOnly = false }, ref) {
  const { control, handleSubmit, reset, trigger, formState: { errors } } = useForm({
    resolver: zodResolver(conditions1Schema),
    mode: 'onChange',
  ...
  return (
    <form onSubmit={(e) => e.preventDefault()} className="space-y-6 mb-5 mt-5">
      <SectionCard title="Previous Medical Conditions (1/6)" icon={<Info className="h-4 w-4" />}>
        <div className="space-y-2">
          <p className="text-sm text-muted-foreground mb-4">Please indicate if you have ever had any of the following conditions:</p>
          <div className="divide-y">
            {fields.map((cond) => (
              <div key={cond.key} className="b flex items-center justify-between gap-4 py-3 border-b-1 border-[#0077B6]/20 ">
