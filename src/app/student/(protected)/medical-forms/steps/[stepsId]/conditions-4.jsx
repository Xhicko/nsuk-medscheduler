...
import { conditions4Schema } from '@/lib/medicalForms'

const fields = [
  { key: 'prev_drug_sensitivity', label: 'Drug sensitivity/allergies' },
  { key: 'prev_dysentery', label: 'Dysentery' },
  { key: 'prev_dizziness', label: 'Dizziness' },
  { key: 'prev_jaundice', label: 'Jaundice' },
]

const Conditions4 = forwardRef(function Conditions4({ formData = {}, onFormChange = () => {}, readOnly = false }, ref) {
  const { control, handleSubmit, reset, trigger, formState: { errors } } = useForm({
    resolver: zodResolver(conditions4Schema),
    mode: 'onChange',
  ...
  return (
    <form onSubmit={(e) => e.preventDefault()} className="space-y-6 mb-5 mt-5">
      <SectionCard title="Previous Medical Conditions (4/6)" icon={<Info className="h-4 w-4" />}>
