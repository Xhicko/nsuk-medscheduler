...
import { conditions5Schema } from '@/lib/medicalForms'
...
const Conditions5 = forwardRef(function Conditions5({ formData = {}, onFormChange = () => {}, readOnly = false }, ref) {
  const { control, handleSubmit, reset, trigger, formState: { errors } } = useForm({
    resolver: zodResolver(conditions5Schema),
    mode: 'onChange',
  ...
  return (
    <form onSubmit={(e) => e.preventDefault()} className="space-y-6 mb-5 mt-5">
      <SectionCard title="Previous Medical Conditions (5/6)" icon={<Info className="h-4 w-4" />}>
